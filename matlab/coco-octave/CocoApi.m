classdef CocoApi
  ...
  methods
    ...
    function download( coco, tarDir, maxn )
      ...
      urls={coco.data.images.coco_url}; urls=urls(o); todo=true(1,n);
      for i=1:n, fs{i}=[tarDir '/' fs{i}]; todo(i)=~exist(fs{i},'file'); end
      fs=fs(todo); urls=urls(todo); n=length(fs); if(n==0), return; end
